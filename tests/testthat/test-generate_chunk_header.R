...
testthat::test_that("The header is well generated", {
    ...
    testthat::expect_identical(
        ...
        expected = "```{r, error = TRUE, results = \"asis\", comment = \"##\", background = \"#E7E7E7\"}"
    )
    testthat::expect_identical(
        object = generate_chunk_header(fig.width = 8.2, fig.height = 10,
                                       dpi = 124, warning = FALSE),
        expected = "```{r, fig.width = 8.2, fig.height = 10, dpi = 124, warning = FALSE}"
    )
})
...
testthat::test_that("The header is well generated", {
    testthat::expect_error(object = generate_chunk_header(eval = TRUE, eval = FALSE))
