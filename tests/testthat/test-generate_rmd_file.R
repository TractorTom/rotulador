testthat::test_that("Everything works with default parameters", {
    exp1 <- readLines(testthat::test_path(
        "template_rmd",
        "default",
        "template_word.Rmd"
    ))
    ...
    exp2 <- readLines(testthat::test_path(
        "template_rmd",
        "default",
        "template_html.Rmd"
    ))
    obj2_a <- generate_rmd_file(
        output_format = "html",
        content = "plot(AirPassengers)"
    )
    obj2_b <- generate_rmd_file(
...
testthat::test_that("Everything works with custom parameters", {
    ...
    exp4 <- readLines(testthat::test_path(
        "template_rmd",
        "custom",
        "template_word_2.Rmd"
    ))
    ...
    exp5 <- readLines(testthat::test_path(
        "template_rmd",
        "custom",
        "template_html_2.Rmd"
    ))
    obj5_a <- generate_rmd_file(
        output_format = "html",
        content = "plot(AirPassengers)",
        eval = TRUE
    )
    obj5_b <- generate_rmd_file(
        output_format = "html_document",
        content = "plot(AirPassengers)",
        eval = TRUE
    )
