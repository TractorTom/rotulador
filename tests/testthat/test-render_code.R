withr::with_envvar(new = c(CLIPR_ALLOW = TRUE), {
    testthat::test_that("Rmd file is correct", {
        cond_skip_clipr <- !clipr::clipr_available()
        testthat::skip_if(
            condition = cond_skip_clipr,
            message = "{clipr} not configured."
        )
    ...
    testthat::test_that("PDF Output file is correct", {
        cond_skip_latex <- !nzchar(latex_engine)
        testthat::skip_if(
            condition = cond_skip_latex,
            message = "Latex not installed."
        )
        cond_skip_clipr <- !clipr::clipr_available()
        testthat::skip_if(
            condition = cond_skip_clipr,
...
print(mtcars)",
        ...
        path_template_pdf <- testthat::test_path("output_data", "output.pdf")
        ...
        path_template_tex <- testthat::test_path("output_data", "output.tex")
    ...
    testthat::test_that("Other output file is correct", {
        cond_skip_clipr <- !clipr::clipr_available()
        testthat::skip_if(
            condition = cond_skip_clipr,
            message = "{clipr} not configured."
...
print(mtcars)",
        ...
        # Check HTML file
        path_html <- file.path(
            render_code(output_format = "html_document"),
            "output.html"
        )
        path_template_html <- testthat::test_path("output_data", "output.html")
        txt_output_html <- htm2txt::gettxt(path_html)
        txt_template_output_html <- htm2txt::gettxt(path_template_html)

        testthat::expect_identical(
            object = txt_output_html,
